            ...
            try:
                ...
                if cnn_similarity >= similarity_threshold:
                    ...
                    if target_gps and current_gps:
                        geo_distance = geodesic(target_gps, current_gps).kilometers
                        if geo_distance <= geo_threshold:
                            # Boost for images within or equal to the threshold
                            if cnn_similarity > 0.48 and geo_distance < 0.1:
                                confidence += 0.3  # Significant boost for high similarity and very close distance
                            else:
                                confidence += 0.05  # Small boost for images within the threshold
                        elif 0.5 < geo_distance <= 1.0:
                            # Graduated decrease for distances between 0.5km and 1.0km
                            decrease = 0.05 + (geo_distance - 0.5) * 0.1
                            confidence -= decrease
                        elif geo_distance > 1.0:
                            # Maximum decrease for distances over 1.0km
                            confidence -= 0.1
                    else:
                        # No adjustment for images without GPS data
                        pass
                    ...
                    similar_images.append((file_path, file_name, confidence, cnn_similarity, object_similarity, current_gps, geo_distance, None, None, None))
                else:
...
def create_montage(similar_images, target_image_path, similarity_threshold, geo_threshold):
    ...
    def add_image_to_montage(img_path, x, y, conf, sim, obj, dist):
        ...
        if dist is not None:
            draw.text((x, text_y + 45), f"Dist: {dist:.2f} km", fill='black', font=font)
        else:
