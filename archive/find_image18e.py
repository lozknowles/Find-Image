...
# - Modified confidence calculation to penalize images more than 0.5km away
# - Updated montage to display confidence, similarity, and distance for each image
# - Adjusted ranking to prioritize nearby images over distant ones with similar visual features
        ...
        if file_name.endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')):
            try:
                ...
                if geo_distance is not None:
                    if geo_distance <= 0.5:
                        confidence += (1 - (geo_distance / 0.5)) * 0.2  # Boost for nearby images
                    else:
                        confidence -= min((geo_distance - 0.5) / 9.5, 0.2)  # Penalty for distant images, max 0.2
...
def create_montage(similar_images, target_image_path, script_info, output_file="montage.png"):
    ...
    for idx, (img_path, _, confidence, similarity, gps, distance) in enumerate(similar_images):
        ...
        draw.text((x_offset + 10, y_offset + image_size[1] + 10), f"Conf = {confidence:.4f}", fill=(0, 0, 0), font=font)
        draw.text((x_offset + 10, y_offset + image_size[1] + 25), f"Sim = {similarity:.4f}", fill=(0, 0, 0), font=font)

        if distance is not None:
            distance_text = f"Dist: {distance:.2f} km"
        else:
            distance_text = "No GPS data"
        draw.text((x_offset + 10, y_offset + image_size[1] + 40), distance_text, fill=(0, 0, 0), font=font)
