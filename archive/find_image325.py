...
def create_montage(similar_images, target_image_path, similarity_threshold, geo_threshold):
    ...
    header_text += "   b) Distance <= 0.5km: +0.05 boost\n"
    header_text += "   c) Distance > 1km: -0.05 penalty\n"
    ...
    for i, (img_path, _, confidence, similarity, object_similarity, gps, geo_distance, _, _, _) in enumerate(similar_images, start=1):
        img = open_image_with_orientation(img_path)
        img = img.convert('RGB')
        ...
        draw.text((x, text_y + 30), f"Obj = {object_similarity:.4f}", fill='black', font=font)
        if gps is not None and geo_distance is not None:
            draw.text((x, text_y + 45), f"Dist: {geo_distance:.2f} km", fill='black', font=font)
        else:
